...
describe("MongoDB Remote service", () => {
  ...
  it("can insert a document", async () => {
    ...
    const service = createService(fetcher, "my-mongodb-service");
    const result = await service.db("my-database").collection<MyDocument>("my-collection").insertOne({
  ...
  it("can insert many documents", async () => {
    ...
    const fetcher = new MockFetcher([
      {
        insertedIds: [{ $oid: "deadbeefdeadbeefdead0001" }, { $oid: "deadbeefdeadbeefdead0002" }],
      },
    ]);
    const service = createService(fetcher, "my-mongodb-service");
    const result = await service
  ...
  it("can count documents", async () => {
    ...
    const service = createService(fetcher, "my-mongodb-service");
    const result = await service.db("my-database").collection<MyDocument>("my-collection").count({}, { limit: 9999 });
